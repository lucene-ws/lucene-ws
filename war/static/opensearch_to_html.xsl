...
<xsl:stylesheet version="1.0"
 ...
 >
	<xsl:output omit-xml-declaration="yes" method="xml" doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-strict.dtd" encoding="UTF-8" media-type="text/html" /> <!-- text/html is needed for the hack (see '.js' below) to work -->
	
	<!-- START SETTINGS -->
	<!-- text used; change this for translation and also some settings -->
	<xsl:variable name="use-doe-hack">true</xsl:variable> <!-- leave this as true to enable a hack requiring the javascript file to handle web browsers that don't support disable-output-escaping (e.g. Mozilla-based ones); leave blank otherwise -->
	<xsl:variable name="t-lang">en</xsl:variable> <!-- the ISO 639 code the the language that the text (the texts below) are in -->
	...
	<xsl:template match="atom:feed | rss/channel | rdf:RDF">
		...
		<html xml:lang="{$language}" lang="{$language}">
			...
			<head>
				...
				<link rel="stylesheet" href="opensearch_to_html.css" type="text/css" title="default" media="screen" />
				<!-- rel links -->
			...
			<body>
				...
				<xsl:choose>
					<xsl:when test="$items">
						...
						<!-- hack for xslt processors that do not respect disable-output-escaping="yes" --><xsl:if test="string-length($use-doe-hack)"><span id="encoding-test" style="display:none"><xsl:text disable-output-escaping="yes">&amp;amp;</xsl:text></span><script type="text/javascript" src="opensearch_to_html.js"></script></xsl:if>
						<!-- result navigation -->
						<p id="nav">
