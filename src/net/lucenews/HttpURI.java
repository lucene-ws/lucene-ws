...
public class HttpURI {
	...
	protected void setUri(String uri) {
		// Logger.getLogger( HttpURI.class ).debug( "STARTED SETTING URI TO " +
		// uri );
		...
		// Logger.getLogger( HttpURI.class ).debug( "FINISHED SETTING URI TO " +
		// uri + ", PATH = " + getPath() );
	}
	...
	public void setPath(String path) {
		...
		if (path != null && path.trim().length() > 0) {
			Iterator<String> iterator = Arrays.asList(path.split("/"))
					.iterator();
	...
	/**
	 * Formats a String suitable for use within a URI. Non-ASCII characters are
	 ...
	 * representation.
	 * 
	 * @param b the byte to be escaped @return the String representation of the
	 * escaped byte
	 */
	public static String escape(byte b) {
		...
		escaped += alphabet[(int) ch]; // convert the nibble to a String
		// Character
	...
	public String toString() {
		...
		if (parameters != null) {
			boolean first = true;
			Iterator<Map.Entry<String, String[]>> entriesIterator = parameters
					.entrySet().iterator();
