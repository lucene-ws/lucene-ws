...
public class XOXOController {
	...
	/**
	 * Transforms a set of properties into an XOXO-formatted &lt;dt&gt; list.
	 * 
	 * @param properties
	 *            The properties to transform
	 * @param document
	 *            The DOM document used to generate elements
	 * @return An XOXO-formatted &lt;dl&gt; element
	 */

	public static Element asElement(LuceneContext c, Properties properties,
			Document document) {
		Logger.getLogger(XOXOController.class).trace(
				"asElement(LuceneContext,Properties,Document)");
	...
	/**
	 * Transforms the given document into an XOXO-formatted &lt;dl&gt; list.
	 * 
	...
	public static Element asElement(LuceneContext c,
			LuceneDocument luceneDocument, Document document) {
		Logger.getLogger(XOXOController.class).trace(
				"asElement(LuceneContext,LuceneDocument,Document)");
	...
	public static String getClassName(LuceneContext c, Field field) {
		Logger.getLogger(XOXOController.class).trace(
				"getClassName(LuceneContext,Field)");
	...
	/**
	 * Transforms the given Atom entry into an XOXO-formatted &lt;dl&gt; list
	 * element. This is typically accomplished by examining the entry's content.
	 * 
	 * @param entry
	 *            The Atom entry to transform
	...
	public static Element asElement(LuceneContext c, Entry entry)
			...
			LuceneException {
		Logger.getLogger(XOXOController.class).trace(
				"asElement(LuceneContext,Entry)");

		Content content = entry.getContent();

		if (content == null) {
			throw new LuceneException("Entry contains no content",
					LuceneResponse.SC_BAD_REQUEST);
		}
		...
		if (content instanceof NodeContent) {
			...
			for (int i = 0; i < nodes.length; i++) {
				...
				if (node.getNodeType() == Node.ELEMENT_NODE) {
					Element dl = (Element) ((Element) node)
							.getElementsByTagName("dl").item(0);
					return dl;
				}
			}
		}
		...
		if (content instanceof TextContent) {
			...
			if (textContent.isType("xhtml")) {
				return (Element) textContent.asDocument().getElementsByTagName(
						"dl").item(0);
			}
		}
	...
	/**
	 * Transforms a given Atom entry into an array of fields.
	 * 
	 * @param entry
	 *            The Atom entry to transform
	...
	public static Field[] asFields(LuceneContext c, Entry entry)
			...
			LuceneException {
		Logger.getLogger(XOXOController.class).trace(
				"asFields(LuceneContext,Entry)");

		return asFields(c, asElement(c, entry));
	}

	/**
	 * Transforms a given Atom entry into a set of properties.
	 * 
	 * @param entry
	 *            The Atom entry to transform
	...
	public static Properties asProperties(LuceneContext c, Entry entry)
			...
			LuceneException {
		Logger.getLogger(XOXOController.class).trace(
				"asProperties(LuceneContext,Entry)");

		return asProperties(c, asElement(c, entry));
	}

	/**
	 * Transforms the given properties into an Atom content object.
	 * 
	 * @param properties
	...
	public static Content asContent(LuceneContext c, Properties properties)
			throws ParserConfigurationException {
		Logger.getLogger(XOXOController.class).trace(
				"asContent(LuceneContext,Properties)");
	 ...
	 * 
	 * @param dl
	 *            A DOM element corresponding to the &lt;dl&gt; list
	 * @return a set of properties
	 * @throws LuceneException
	 */

	public static Properties asProperties(LuceneContext c, Element dl)
			throws LuceneException {
		Logger.getLogger(XOXOController.class).trace(
				"asProperties(LuceneContext,Element)");
	...
	/**
	 * Transforms a given &lt;dl&gt; list element into an array of fields.
	 * 
	 * @param dl
	 *            The &lt;dl&gt; list element
	 * @return An array of fields
	...
	public static Field[] asFields(LuceneContext c, Element dl)
			throws LuceneParseException {
		Logger.getLogger(XOXOController.class).trace(
				"asFields(LuceneContext,Element)");
			...
			case Node.ELEMENT_NODE:
				...
				if (element.getTagName().equals("dt")) {
					if (state == 't') {
						...
						fields.add(asField(c, name, value, indexed, stored,
								tokenized, termVectorStored));
						state = 't';
					}
				}
				...
				if (element.getTagName().equals("dd")) {
					if (state == 'd') {
						...
						if (name != null && value != null) {
							fields.add(asField(c, name, value, stored, indexed,
									tokenized, termVectorStored));
						}
						value = null;
						state = 't';
	 ...
	 * parameters. If the requested field is not stored, not indexed and not
	 * tokenized (a dud, so to speak), a field as constructed by
	...
	protected static Field asField(LuceneContext c, String name, String value,
			boolean stored, boolean indexed, boolean tokenized,
			boolean termVectorStored) {
		Logger
				.getLogger(XOXOController.class)
				.trace(
						"asField(LuceneContext,String,String,boolean,boolean,boolean,boolean)");

		Field.Store store = stored ? Field.Store.YES : Field.Store.NO;
		Field.Index index = Field.Index.NO;
		if (indexed) {
			index = tokenized ? Field.Index.TOKENIZED
					: Field.Index.UN_TOKENIZED;
		}
		Field.TermVector termVector = termVectorStored ? Field.TermVector.YES
				: Field.TermVector.NO;
