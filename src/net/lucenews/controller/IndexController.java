...
import net.lucenews.view.XMLView;
...
public class IndexController extends Controller {
	...
	public static void doDelete(LuceneContext c) throws TransformerException,
		...
		for (int i = 0; i < indices.length; i++) {
			...
			if (!index.delete()) {
				throw new IOException("Index '" + index.getName()
	...
	public static void doGet(LuceneContext c)
			throws ParserConfigurationException, InvalidIdentifierException,
			DocumentNotFoundException, IndicesNotFoundException,
			ParserConfigurationException, TransformerException, IOException,
			DocumentAlreadyExistsException, InsufficientDataException,
			ParseException, OpenSearchException {
	...
	public static void doPut(LuceneContext c) throws IndicesNotFoundException,
			ParserConfigurationException, SAXException, IOException,
			LuceneException, TransformerException,
		...
		for (int i = 0; i < entries.length; i++) {
			...
			if (!request.hasIndexName(name)) {
				throw new LuceneException("Index '" + name
						+ "' not mentioned in URL");
			}
	...
	public static void doPost(LuceneContext c) throws IllegalActionException,
			TransformerException, DocumentsAlreadyExistException,
			IndicesNotFoundException, ParserConfigurationException,
			IOException, SAXException, LuceneException, AtomParseException {
		...
		if (response.getStatus() == response.SC_CREATED) {
			...
			response.addHeader("Content-Location", service.getDocumentURI(
					request, indexNames, documentIDs).toString());
		} else {
		...
		// TODO: A representation of the added entries must be returned as per
		// the APP
		if (addedDocuments.size() == 1) {
			...
			entry.addLink(Link.Edit(service.getDocumentURI(request, document)
					.toString()));
			...
			while (iterator.hasNext()) {
				...
				Entry entry = DocumentController.asEntry(c, document);
				entry.addLink(Link.Edit(service.getDocumentURI(request,
						document).toString()));
				feed.addEntry(entry);
			}
	...
	public static void doOptimize(LuceneContext c)
			...
			IndicesNotFoundException, IOException {
		Logger.getLogger(IndexController.class).trace(
				"doOptimize(LuceneContext)");
	...
	public static void doTagCloud(LuceneContext c)
		...
		Iterator<Map.Entry<String, Integer>> iterator = cloud.entrySet()
				.iterator();
		while (iterator.hasNext()) {
			...
			double size = (double) (count - minimum)
					/ (double) (maximum - minimum);
			size = minimum_font_size + size
					* (maximum_font_size - minimum_font_size);
			size = (int) size;
