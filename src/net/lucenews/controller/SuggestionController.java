...
public class SuggestionController extends Controller {
    ...
    public static void doSuggest (LuceneContext c, Query query, OpenSearchResponse response)
        throws IndicesNotFoundException, IOException
    {
        response.addQueries( getOpenSearchQuerySuggestions( c, query, c.getOpenSearchQuery().getSearchTerms() ) );
    }
    ...
    public static List<OpenSearchQuery> getOpenSearchQuerySuggestions (LuceneContext c, Query original, String searchTerms)
        throws IndicesNotFoundException, IOException
    {
        ...
        while ( iterator.hasNext() ) {
            ...
            if ( reconstructor == null || searchTerms == null ) {
                ...
                openSearchQuery.setSearchTerms( reconstructor.reconstruct( query, searchTerms ) );
            }
            
            openSearchQuery.setTotalResults( searcher.search( query, c.getFilter() ).length() );
        ...
        throws IOException
    {
        ...
        if ( synonymExpander != null ) {
            Logger.getLogger(SuggestionController.class).trace("suggestSynonyms");
...
/**
 * A class used to assist sorting of OpenSearchQuery objects according 
 * to their respective totalResults values.
 */
