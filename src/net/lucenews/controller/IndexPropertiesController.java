...
public class IndexPropertiesController extends Controller {
	...
	public static void doPut(LuceneContext c) throws SAXException,
		...
		if (updated) {
			response.addHeader("Location", service.getIndexPropertiesURI(
					request, indexNamesBuffer.toString()).toString());
		}
	...
	public static void doPost(LuceneContext c) throws SAXException,
			TransformerException, ParserConfigurationException, IOException,
			IndexNotFoundException, LuceneException, AtomParseException {
		LuceneWebService service = c.getService();
		LuceneIndexManager manager = service.getIndexManager();
	...
	public static Entry asEntry(LuceneContext c, LuceneIndex index,
			boolean authorRequired) throws ParserConfigurationException,
			IOException {
		LuceneWebService service = c.getService();
		LuceneIndexManager manager = service.getIndexManager();
		LuceneRequest request = c.getRequest();
		...
		entry.setID(c.getService().getIndexPropertiesURI(request, index)
				.toString());
		...
		/*
		 * if ( index.hasAuthor() ) { entry.addAuthor( new Author(
		 * index.getAuthor() ) ); } else
		 */
		if (authorRequired) {
