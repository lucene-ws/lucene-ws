...
import net.lucenews.opensearch.OpenSearchUrl;
...
public class OpenSearchController extends Controller {
	...
	public static void doGet(LuceneContext c) throws IndicesNotFoundException,
		...
		if (description.getShortName() == null) {
			description.setShortName(ServletUtils.joined(ServletUtils
					.objectsMapped("getTitle", indices)));
		}

		// Description
		if (description.getDescription() == null) {
			description.setDescription("OpenSearch description for "
		 ...
		 * atomUrl.addParameter( "count", "{count?}" ); atomUrl.addParameter(
		 * "startIndex", "{startIndex?}" ); atomUrl.addParameter( "startPage",
		 * "{startPage?}" ); atomUrl.addParameter( "language", "{language?}" );
		 ...
		 * rssUrl.addParameter( "count", "{count?}" ); rssUrl.addParameter(
		 * "startIndex", "{startIndex?}" ); rssUrl.addParameter( "startPage",
		 * "{startPage?}" ); rssUrl.addParameter( "language", "{language?}" );
		 ...
		 * OpenSearchUrl suggestUrl = new OpenSearchUrl(); suggestUrl.setType(
		 * "application/x-suggestions+json" ); HttpURI suggestTemplate =
		 * template.clone(); suggestTemplate.setParameters("");
		 * suggestTemplate.setParameter("searchTerms", "{searchTerms}");
		 * suggestTemplate.addPath("suggest"); suggestUrl.setTemplate(
		 * suggestTemplate.toString() ); description.addUrl( suggestUrl );
		 */
		...
		Iterator<Map.Entry<String, Charset>> charsetIterator = Charset
				.availableCharsets().entrySet().iterator();
	...
	public static void addDefaults(OpenSearchDescription description,
			LuceneIndex index) throws IOException {
		...
		// SyndicationRight
		description.setSyndicationRight(index.getProperty(ns
				+ "syndicationright"));
