...
public class OpenSearchResponse {
	...
	public Link getRelatedLink(String rel) {
		...
		while (iterator.hasNext()) {
			...
			if (link.getRel() != null
					&& link.getRel().toLowerCase().trim().equals(
							rel.toLowerCase().trim())) {
				return link;
			}
		}
		return null;
	...
	public OpenSearchQuery getRelatedQuery(String role) {
		...
		while (iterator.hasNext()) {
			...
			if (query.getRole() != null
					&& query.getRole().toLowerCase().trim().equals(
	...
	public Element asElement(Document document, OpenSearch.Format format,
		...
		if (format == OpenSearch.ATOM) {
			...
			element.setAttribute("xmlns:opensearch",
					"http://a9.com/-/spec/opensearch/1.1/");
			element.setAttribute("xmlns:relevance",
			...
			if (totalResults == null && requestQuery != null) {
				totalResults = requestQuery.getTotalResults();
			}
			if (totalResults != null) {
				element
						.appendChild(asElementNS(document,
								"http://a9.com/-/spec/opensearch/1.1/",
								"opensearch:totalResults", String
										.valueOf(totalResults)));
			}

			// startIndex
			Integer startIndex = getStartIndex();
			if (startIndex == null && requestQuery != null) {
				startIndex = requestQuery.getStartIndex();
			}
			if (startIndex != null) {
				element.appendChild(asElementNS(document,
						"http://a9.com/-/spec/opensearch/1.1/",
						"opensearch:startIndex", String.valueOf(startIndex)));
			}

			// itemsPerPage
			Integer itemsPerPage = getItemsPerPage();
			if (itemsPerPage == null && requestQuery != null) {
				itemsPerPage = requestQuery.getCount();
			}
			...
			Iterator<OpenSearchResult> resultsIterator = getResults()
					.iterator();
		...
		if (format == OpenSearch.RSS) {
			...
			if (getLink() != null && getLink().getHref() != null) {
				channel.appendChild(asElement(document, "link", getLink()
						.getHref()));
			}

			// description
			if (getDescription() != null) {
				channel.appendChild(asElement(document, "description",
						getDescription()));
			}

			// totalResults
			if (getTotalResults() != null) {
				channel.appendChild(asElementNS(document,
						"http://a9.com/-/spec/opensearch/1.1/",
						"opensearch:totalResults", String
	...
	protected Element asElementNS(Document document, String namespaceURI,
			String qualifiedName, String value) throws OpenSearchException {
