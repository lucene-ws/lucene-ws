...
public class QueryReconstructor {
    ...
    public String reconstruct (Query query, String searchTerms) {
        ...
        cursor = 0;
        Logger.getLogger(this.getClass()).debug("reconstructing '" + searchTerms + "'");
        reconstruct( query, searchTerms, buffer, "  " );
        buffer.append( searchTerms.substring( cursor, searchTerms.length() ) );
        return buffer.toString();
    }
    
    protected void reconstruct (Query query, String searchTerms, StringBuffer buffer) {
        reconstruct( query, searchTerms, buffer, "" );
    }
    ...
    protected void reconstruct (BooleanQuery booleanQuery, String searchTerms, StringBuffer buffer, String indent) {
    ...
    protected void reconstruct (TokenTermQuery tokenTermQuery, String searchTerms, StringBuffer buffer, String indent) {
        ...
        Logger.getLogger(this.getClass()).debug(indent+"reconstructing TokenTermQuery: " + tokenTermQuery + " (cursor:" + cursor + ", beginColumn: " + beginColumn + ")");
    ...
    protected void reconstruct (TokenBooleanQuery tokenBooleanQuery, String searchTerms, StringBuffer buffer, String indent) {
        int beginColumn = tokenBooleanQuery.getToken().beginColumn;
        int endColumn   = tokenBooleanQuery.getToken().endColumn;
        
        Logger.getLogger(this.getClass()).debug(indent+"reconstructing TokenBooleanQuery: " + tokenBooleanQuery);
        
        // append the gap
        buffer.append( searchTerms.substring( cursor, beginColumn ) );
        
        // get the TokenTermQuery instances
        TokenTermQuery[] tokenTermQueries = getTokenTermQueries( tokenBooleanQuery ).toArray( new TokenTermQuery[]{} );
    ...
    public String reconstructTermQueries (Query alternate, String searchTerms) {
        ...
        Logger.getLogger(this.getClass()).debug("Reconstructing query ["+alternate+"] using '"+searchTerms+"'");
        
        List<TokenTermQuery>     tokenTermQueries  = getTokenTermQueries( alternate );
        Iterator<TokenTermQuery> tokenTermIterator = tokenTermQueries.iterator();
        
        int cursor = 0;
        while ( tokenTermIterator.hasNext() ) {
            TokenTermQuery tokenTermQuery = tokenTermIterator.next();
            Logger.getLogger(this.getClass()).debug("tokenTermQuery "+tokenTermQuery + ", token: "+tokenTermQuery.getToken());
            reconstructed.append( searchTerms.substring( cursor, tokenTermQuery.getToken().beginColumn ) );
    ...
    public String reconstructBooleanQueries (Query alternate, String original) {
        ...
        Iterator<TokenBooleanQuery> tokenBooleanIterator = tokenBooleanQueries.iterator();
    ...
    /**
     * Gets a list of all instances of TokenTermQuery within the 
    ...
    /**
     * Gets a list of all instances of TokenTermQuery within the 
     * context of this BooleanQuery.
     */
    
    public static List<TokenTermQuery> getTokenTermQueries (BooleanQuery booleanQuery) {
        List<TokenTermQuery> tokenTermQueries = new ArrayList<TokenTermQuery>();
    ...
    /**
     * Gets a list of all instances of TokenBooleanQuery within the 
     * context of this Query.
     */
    ...
    public static List<TokenBooleanQuery> getTokenBooleanQueries (Query query) {
        ...
        if ( query instanceof BooleanQuery ) {
            tokenBooleanQueries.addAll( getTokenBooleanQueries( (BooleanQuery) query ) );
        }
        
        // sort according to beginning column
        Collections.sort( tokenBooleanQueries, new TokenBooleanQueryComparator() );
    ...
    public static List<TokenBooleanQuery> getTokenBooleanQueries (BooleanQuery booleanQuery) {
        List<TokenBooleanQuery> tokenBooleanQueries = new ArrayList<TokenBooleanQuery>();
        ...
        BooleanClause[] clauses = booleanQuery.getClauses();
        for ( int i = 0; i < clauses.length; i++ ) {
            tokenBooleanQueries.addAll( getTokenBooleanQueries( clauses[i].getQuery() ) );
        }
