...
public class LuceneDocument {
	...
	public Calendar getLastModified() throws java.text.ParseException,
			InsufficientDataException, IOException {
	...
	public List<Field> getFields() {
		...
		/**
		 * 
		 * NOTE: The following lines will only work when compiling against
		 * Lucene-Core 2.0.1. Until that is officially launched, we must deal
		 * with 2.0.0 API.
		 * 
		 * List<?> objects = document.getFields(); Iterator<?> iterator =
		 * objects.iterator(); while (iterator.hasNext()) { Object object =
		 * iterator.next(); if (object instanceof Field) { fields.add( (Field)
		 * object ); } }
		 */
