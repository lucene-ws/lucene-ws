...
/**
 * <p>
 * Generates alternative queries by examining term(s) from a TermQuery or a
 * BooleanQuery with TermQuery's, and determines if a term can be replaced with
 * a similar spelled term which can produce more hits.
 * <p>
...
public class DidYouMeanQueryGenerator {
	...
	/**
	 * Tries to find a query which will give more hits using the
	 * {@link FuzzyQuery} algorithm (<a
	 * href="http://www.levenshtein.net/">Levenshtein distance</a>) along with
	 * document frequency for a term.
	 * 
	 * @param useFreqCount
	 *            Set to <tt>true</tt> to only replace terms which have a
	 *            higher document frequency count (and would give more hits)
	 *            than the replaced original term.
	 * @param useDefaultBoost
	 *            Set to <tt>true</tt> to give all replaced term queries a
	 *            boost of 1.0f instead of the boost computed from the edit
	 *            distance (suitable for human-readable query.toString(field)
	 *            result).
	 * @return If and only if a more suitable query can be calculated, a new
	 *         {@link Query} is returned. In all other cases the original query
	 *         is returned.
	 */

	public Query getQuerySuggesstion(boolean useFreqCount,
			boolean useDefaultBoost) throws IOException {
		List<Query> suggestions = getQuerySuggestions(useFreqCount,
				useDefaultBoost);
	...
	/**
	 * Gets a list of query suggestions. Ordered by highest cumulative boost to
	 * lowest.
	 * 
	 * @param useFreqCount
	 * @param useDefaultBoost
	...
	public List<Query> getQuerySuggestions(boolean useFreqCount,
			boolean useDefaultBoost) throws IOException {
		List<Query> suggestions = new ArrayList<Query>();

		if (originalQuery instanceof TermQuery) {
			suggestions.addAll(getBetterTermQueries((TermQuery) originalQuery,
					useFreqCount, useDefaultBoost));
		}

		if (originalQuery instanceof BooleanQuery) {
			suggestions.addAll(recursiveExtract((BooleanQuery) originalQuery,
					useFreqCount, useDefaultBoost));
		}
	...
	/**
	 * Recursive extraction of the BooleanQuery in order to find all
	 * TermQuery's. Returns a list sorted from greatest boost to least
	 * 
	 * @param bq
	 * @param useFreqCount
	 * @param useDefaultBoost
	...
	private List<BooleanQuery> recursiveExtract(BooleanQuery bq,
			boolean useFreqCount, boolean useDefaultBoost) throws IOException {
		...
		for (int i = 0; i < clauses.length; i++) {
			...
			if (!clause.isProhibited()
				...
				queries[i] = recursiveExtract((BooleanQuery) clause.getQuery(),
						useFreqCount, useDefaultBoost).toArray(new Query[] {});

			if (!clause.isProhibited()
					&& clause.getQuery() instanceof TermQuery)
				queries[i] = getBetterTermQueries(
						(TermQuery) clause.getQuery(), useFreqCount,
						useDefaultBoost).toArray(new Query[] {});
		}
	...
	private List<TermQuery> getBetterTermQueries(TermQuery tq,
			boolean useFreqCount, boolean useDefaultBoost) throws IOException {
...
class ScaledBoostQueryComparator implements Comparator<Query> {
	...
	/**
	 * Returns the scaled boost of the query. Useful when sorting by a
	 * de-normalized scale factor.
	 */
