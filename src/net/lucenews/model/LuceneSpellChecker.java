...
public class LuceneSpellChecker extends SpellChecker {
	...
	public List<Query> suggestSimilar(Query query) throws IOException {
	...
	public List<Query> suggestSimilar(Query query, int num_sug, IndexReader ir,
		...
		Logger.getLogger(this.getClass()).trace(
				"suggestSimilar(Query,int,IndexReader,boolean);");

		List<Query> queries = new LinkedList<Query>();

		if (query instanceof BooleanQuery) {
			return suggestSimilar((BooleanQuery) query, num_sug, ir,
					morePopular);
		}
	...
	public List<Query> suggestSimilar(BooleanQuery query, int num_sug,
			IndexReader ir, boolean morePopular) throws IOException {
		Logger.getLogger(this.getClass()).trace(
				"suggestSimilar(BooleanQuery,int,IndexReader,boolean);");
		...
		for (int i = 0; i < clauses.length; i++) {
			...
			clauseQuerySuggestions[i] = suggestSimilar(clause.getQuery(),
					num_sug, ir, morePopular).toArray(new Query[] {});
		}
		...
		while (indices[0] <= clauseQuerySuggestions[0].length) {
			...
			for (int i = 0; i < indices.length; i++) {
				int j = indices[i];
				if (clauseQuerySuggestions[i].length > 0
						&& clauseQuerySuggestions[i].length > j) {
	...
	public List<Query> suggestSimilar(TermQuery query, int num_sug,
			IndexReader ir, boolean morePopular) throws IOException {
		Logger.getLogger(this.getClass()).trace(
				"suggestSimilar(TermQuery,int,IndexReader,boolean);");
		...
		String[] suggestions = suggestSimilar(word, num_sug, ir, field,
				morePopular);
