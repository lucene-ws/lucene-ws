...
public class LuceneQueryParser extends QueryParser {
	 ...
	 * ParseException { Query query = super.getFieldQuery( field, queryText );
	 * 
	 * Logger.getLogger(this.getClass()).debug("Getting FieldQuery for \"" +
	 * field + "\", \"" + queryText + "\"");
	 * 
	 * if (query instanceof TermQuery) { TermQuery termQuery = (TermQuery)
	 * query; query = new TokenTermQuery( termQuery.getTerm(), getToken(0) ); }
	 * 
	 * try { LuceneSynonymExpander expander = getSynonymExpander(); // construct
	 * a new expander if we have not already done so if ( expander == null ) {
	 * expander = new LuceneSynonymExpander(); expander.setSearcher(
	 * getSynonymSearcher() ); expander.setAnalyzer( getAnalyzer() );
	 * setSynonymExpander( expander ); }
	 * 
	 * query = expander.expand(query);
	 * 
	 * if (query instanceof BooleanQuery) { BooleanQuery booleanQuery =
	 * (BooleanQuery) query; BooleanClause[] clauses =
	 * booleanQuery.getClauses();
	 * 
	 * TokenBooleanQuery tokenBoolean = new TokenBooleanQuery(
	 * booleanQuery.isCoordDisabled(), getToken( 0 ) ); for (int i = 0; i <
	 * clauses.length; i++) { tokenBoolean.add( clauses[ i ] ); }
	 * 
	 * query = tokenBoolean; } } catch (Exception e) { }
	 * 
	 * return query; }
	 */
		...
		if (field.equals(getField())) {
			...
			if (getFields().length == 1) {
				...
				for (int i = 0; i < getFields().length; i++) {
					...
					Query query = getFieldQuery(getFieldName(subfield),
							queryText);
					if (hasFieldBoost(subfield)) {
						query.setBoost(getFieldBoost(subfield));
					}
				...
				Logger.getLogger(this.getClass()).debug(
						"Returning " + booleanQuery.getClass());
				return booleanQuery;
			}
		} else {
			...
			Logger.getLogger(this.getClass()).debug(
					"Getting FieldQuery for \"" + field + "\", \"" + queryText
							+ "\"");
			...
			try {
				...
				if (query instanceof BooleanQuery) {
					...
					TokenBooleanQuery tokenBoolean = new TokenBooleanQuery(
							booleanQuery.isCoordDisabled(), getToken(0));
	...
	/**
	 * protected Query getFieldQuery (String field, String queryText) throws
	 * ParseException { if ( field.equals( getField() ) ) { if (
	 * getFields().length == 1 ) { String subfield = getField( 0 ); Query query =
	 * getFieldQuery( getFieldName( subfield ), queryText ); if ( hasFieldBoost(
	 * subfield ) ) { query.setBoost( getFieldBoost( subfield ) ); } return
	 * query; } else { BooleanQuery booleanQuery = new BooleanQuery(); for ( int
	 * i = 0; i < getFields().length; i++ ) { String subfield = getField( i );
	 * Query query = getFieldQuery( getFieldName( subfield ), queryText ); if (
	 * hasFieldBoost( subfield ) ) { query.setBoost( getFieldBoost( subfield ) ); }
	 * booleanQuery.add( query, getDefaultOccur() ); } return booleanQuery; } }
	 * else { Query query = super.getFieldQuery( field, queryText );
	 * 
	 * Logger.getLogger(this.getClass()).debug("Getting FieldQuery for \"" +
	 * field + "\", \"" + queryText + "\"");
	 * 
	...
	protected Query getFuzzyQuery(String field, String termStr,
		...
		if (field.equals(getField())) {
			if (getFields().length == 1) {
				String subfield = getField(0);
				Query query = getFuzzyQuery(getFieldName(subfield), termStr,
						minSimilarity);
				if (hasFieldBoost(subfield)) {
					query.setBoost(getFieldBoost(subfield));
				}
				...
				for (int i = 0; i < getFields().length; i++) {
					String subfield = getField(i);
					Query query = getFuzzyQuery(getFieldName(subfield),
	...
	protected Query getPrefixQuery(String field, String termStr)
		...
		if (field.equals(getField())) {
			if (getFields().length == 1) {
				...
				for (int i = 0; i < getFields().length; i++) {
					String subfield = getField(i);
					Query query = getPrefixQuery(getFieldName(subfield),
							termStr);
					if (hasFieldBoost(subfield)) {
						query.setBoost(getFieldBoost(subfield));
					}
	...
	protected Query getWildcardQuery(String field, String termStr)
		...
		if (field.equals(getField())) {
			if (getFields().length == 1) {
				...
				for (int i = 0; i < getFields().length; i++) {
					String subfield = getField(i);
					Query query = getWildcardQuery(getFieldName(subfield),
							termStr);
					if (hasFieldBoost(subfield)) {
						query.setBoost(getFieldBoost(subfield));
					}
	...
	protected Query getRangeQuery(String field, String part1, String part2,
		...
		if (fields.equals(getField())) {
			if (getFields().length == 1) {
				String subfield = getField(0);
				Query query = getRangeQuery(getFieldName(subfield), part1,
						part2, inclusive);
				if (hasFieldBoost(subfield)) {
					query.setBoost(getFieldBoost(subfield));
				}
	...
	/**
	 * protected Query getRangeQuery (String field, String part1, String part2,
	 * boolean inclusive) throws ParseException { return new
	 * ConstantScoreRangeQuery( field, part1, part2, inclusive, inclusive ); }
	 */
	...
	protected static Float getFieldBoost(String field) {
		...
		if (field.indexOf("^") >= 0) {
			...
			Logger.getLogger(LuceneQueryParser.class).debug(
					"'" + field + "' contains no caret");
			return null;
		}
	}
