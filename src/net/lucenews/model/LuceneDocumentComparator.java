...
public class LuceneDocumentComparator implements Comparator<LuceneDocument> {
	...
	public int compare(LuceneDocument document1, LuceneDocument document2) {
		...
		try {
			...
			Integer identifiersCompared = compareIdentifiers(document1,
					document2);
	...
	protected Integer compareUpdated(LuceneDocument document1,
			LuceneDocument document2) throws IOException {
		...
		if (!index1.hasLastModifiedFieldName()
				|| !index2.hasLastModifiedFieldName()) {
			return null;
		}

		if (!index1.hasLastModified(document1)
				&& !index2.hasLastModified(document2)) {
			return null;
		}
	...
	protected Integer compareIdentifiers(LuceneDocument document1,
			LuceneDocument document2) throws IOException {
		LuceneIndex index1 = document1.getIndex();
		LuceneIndex index2 = document2.getIndex();
		...
		if (!index1.hasIdentifierFieldName()
				|| !index2.hasIdentifierFieldName()) {
			return null;
		}

		if (!index1.isDocumentIdentified(document1)
				&& !index2.isDocumentIdentified(document2)) {
			return null;
		}
