...
public class OrderedTermsFuzzyQuery extends MultiTermQuery {
	...
	private static final long serialVersionUID = -5781675491724206475L;
	 ...
	 *            the IndexReader
	 * @param useFreqOrder
	 *            if <tt>true</tt>, return list will only contain terms which
	 *            will give more hits
	 * @return List<TermQuery>
	 */

	public List<TermQuery> bestOrderRewrite(IndexReader reader,
			boolean useFreqOrder) throws IOException {
		...
		try {
			do {
				...
				if ((t != null && !t.text().equals(term.text()))
					...
					// set the boost
					// FilteredTermEnum.difference() was protected in revisions
					// < 150572,
		...
		// Collections.sort( terms, (Comparator<TermQuery>) new
		// FuzzyTermScoreComparator( term ) );
