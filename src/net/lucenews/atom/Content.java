...
public abstract class Content {
	...
	protected static Node[] childNodes(Element element)
		...
		if (mode != null && mode.equals("base64")) {
			...
			t.transform(new StreamSource(new StringReader("<wrapper>" + content
					+ "</wrapper>")), new DOMResult(d));
	...
	/**
	 * Extracts the string representation of the child nodes. Adapts to base64
	 * encoded data whenever present.
	 */
	...
	protected static String childNodesToStringBase64(Element element)
			...
			AtomParseException {
		try {
			return new String(new Base64().decode(toString(
					element.getChildNodes()).getBytes()), "UTF-8");
	...
	/**
	 * Parses a DOM element for Atom content. Follows the specifications
	 * outlined at http://www.atomenabled.org/developers/syndication/ Feel free
	...
	public static Content parse(Element element)
		...
		if (!element.getTagName().equals("content"))
			throw new AtomParseException("Invalid content tag name: \""
					+ element.getTagName() + "\"");
		 ...
		 * where the content can be found. The type attribute, if present, is
		...
		if (type != null)
			if (type.endsWith("+xml") || type.endsWith("/xml"))
				return new net.lucenews.atom.NodeContent(element
						.getChildNodes());
