...
import net.lucenews.model.LuceneQueryParser;
...
public class ServletUtils {
	...
	public static void prepareContext(LuceneContext c)
		...
		if (c.getOpenSearchQuery() == null) {
			...
			if (searchTerms == null) {
				searchTerms = request.getCleanParameter("searchTerms");
			}
			if (searchTerms == null) {
				searchTerms = request.getCleanParameter("query");
			}
			if (searchTerms == null) {
				searchTerms = request.getCleanParameter("q");
			}
			...
			if (startIndex == null) {
				startIndex = request.getIntegerParameter("startIndex");
			}
			if (startIndex == null) {
				startIndex = request.getIntegerParameter("start_index");
			}
			if (startIndex == null) {
				startIndex = request.getIntegerParameter("start");
			}
			if (startIndex == null) {
				startIndex = request.getIntegerParameter("offset");
			}
			...
			Integer count = null;
			if (count == null) {
				count = request.getIntegerParameter("count");
			}
			if (count == null) {
				count = request.getIntegerParameter("limit");
			}
			if (count == null) {
				count = request.getIntegerParameter("itemsPerPage");
			}
			if (count == null) {
				count = request.getIntegerParameter("items_per_page");
			}
			if (count == null) {
				count = request.getIntegerParameter("entriesPerPage");
			}
			if (count == null) {
				count = request.getIntegerParameter("entries_per_page");
			}
			...
			if (totalResults == null) {
				totalResults = request.getIntegerParameter("totalResults");
			}
			if (totalResults == null) {
				totalResults = request.getIntegerParameter("total_results");
			}
			if (totalResults == null) {
				totalResults = request.getIntegerParameter("total");
			}
			...
			if (language == null) {
				language = request.getCleanParameter("language");
			}
			if (language == null) {
				language = request.getCleanParameter("lang");
			}
			if (language == null) {
				language = request.getCleanParameter("locale");
			} // This needs fixing
			...
			if (inputEncoding == null) {
				inputEncoding = request.getCleanParameter("inputEncoding");
			}
			if (inputEncoding == null) {
				inputEncoding = request.getCleanParameter("input_encoding");
			}
			if (inputEncoding == null) {
				inputEncoding = request.getCleanParameter("input");
			}
			...
			if (outputEncoding == null) {
				outputEncoding = request.getCleanParameter("outputEncoding");
			}
			if (outputEncoding == null) {
				outputEncoding = request.getCleanParameter("output_encoding");
			}
			if (outputEncoding == null) {
				outputEncoding = request.getCleanParameter("output");
			}
		...
		if (c.getAnalyzer() == null) {
			...
			if (analyzer == null) {
				analyzer = LuceneUtils.parseAnalyzer(request
						.getCleanParameter("analyzer"));
			}
			if (analyzer == null) {
		...
		if (c.getFilterAnalyzer() == null) {
			...
			if (analyzer == null) {
				analyzer = LuceneUtils.parseAnalyzer(request
						.getCleanParameter("filterAnalyzer"));
			}
		...
		if (c.getDefaultOperator() == null) {
			...
			if (defaultOperator == null) {
				defaultOperator = LuceneUtils.parseOperator(request
						.getCleanParameter("defaultOperator"));
			}

			// default_operator
			if (defaultOperator == null) {
				defaultOperator = LuceneUtils.parseOperator(request
						.getCleanParameter("default_operator"));
			}

			// operator
			if (defaultOperator == null) {
				defaultOperator = LuceneUtils.parseOperator(request
						.getCleanParameter("operator"));
			}
		...
		if (c.getDefaultFields() == null) {
			...
			if (defaultFields == null) {
				defaultFields = ServletUtils.split(request
						.getCleanParameterValues("defaultFields"));
			}
			if (defaultFields == null) {
				defaultFields = ServletUtils.split(request
						.getCleanParameterValues("defaultField"));
			}
			if (defaultFields == null) {
				defaultFields = ServletUtils.split(request
						.getCleanParameterValues("default_fields"));
			}
			if (defaultFields == null) {
				defaultFields = ServletUtils.split(request
						.getCleanParameterValues("default_field"));
			}
			if (defaultFields == null) {
				defaultFields = ServletUtils.split(request
						.getCleanParameterValues("defaults"));
			}
			if (defaultFields == null) {
				defaultFields = ServletUtils.split(request
						.getCleanParameterValues("default"));
			}
		...
		if (c.getOpenSearchFormat() == null) {
			...
			if (format == null) {
				try {
					format = OpenSearch.getFormat(request
							.getParameter("format"));
				} catch (Exception e) {
				}
			}
		...
		// locale
		if (c.getLocale() == null) {
			c.setLocale(ServletUtils.parseLocale(request
					.getCleanParameter("locale")));
		}

		// suggest synonyms
		if (c.suggestSynonyms() == null) {
			Boolean suggestSynonyms = null;
			if (suggestSynonyms == null) {
				suggestSynonyms = request.getBooleanParameter("synonyms");
			}
			if (suggestSynonyms == null) {
				suggestSynonyms = service
						.getBooleanProperty("suggest.synonyms");
			}
			c.suggestSynonyms(suggestSynonyms);

			// Synonym expander
			if (suggestSynonyms != null && suggestSynonyms
					&& c.getSynonymExpander() == null) {
				String synonymsIndex = null;

				if (synonymsIndex == null) {
					synonymsIndex = service
							.getCleanProperty("suggest.synonyms.index");
				}

				if (synonymsIndex != null) {
					LuceneSynonymExpander synonymExpander = new LuceneSynonymExpander();
					synonymExpander.setAnalyzer(c.getAnalyzer());
					synonymExpander.setSearcher(manager.getIndex(synonymsIndex)
							.getIndexSearcher());
		...
		if (c.suggestSpelling() == null) {
			...
			if (suggestSpelling == null) {
				suggestSpelling = request.getBooleanParameter("spelling");
			}
			if (suggestSpelling == null) {
				suggestSpelling = service
						.getBooleanProperty("suggest.spelling");
			}
			...
			if (suggestSpelling != null && suggestSpelling) {
				...
				if (spellCheckIndex == null) {
					spellCheckIndex = service
							.getProperty("suggest.spelling.index");
				}
				if (spellCheckIndex == null) {
					spellCheckIndex = "spelling";
				}

				if (spellCheckIndex != null) {
					LuceneSpellChecker spellChecker = new LuceneSpellChecker(
							manager.getIndex(spellCheckIndex)
									.getLuceneDirectory());
		...
		if (c.suggestSimilar() == null) {
			...
			if (suggestSimilar == null) {
				suggestSimilar = request.getBooleanParameter("similar");
			}
			if (suggestSimilar == null) {
				suggestSimilar = service.getBooleanProperty("suggest.similar");
			}
		...
		if (c.getQueryParser() == null) {
			if (c.getDefaultFields() != null && c.getAnalyzer() != null) {
				String fakeDefaultField = "dd8fc45d87f91c6f9a9f43a3f355a94a"; // MD5
				// hash
				// of
				// "boat"
				LuceneQueryParser queryParser = new LuceneQueryParser(
						fakeDefaultField, c.getAnalyzer());
				// LuceneQueryParser queryParser = new LuceneQueryParser(
				// c.getDefaultFields()[ 0 ], c.getAnalyzer() );
		...
		if (c.getFilter() == null && c.getQueryParser() != null) {
			...
			if (filterString == null) {
				filterString = request.getCleanParameter("searchFilter");
			}
			;
			if (filterString == null) {
				filterString = request.getCleanParameter("search_filter");
			}
			;
			if (filterString == null) {
				filterString = request.getCleanParameter("filter");
			}
			;
			...
			if (c.getFilterAnalyzer() == null) {
				...
				filterParser = new QueryParser(c.getQueryParser().getField(), c
						.getFilterAnalyzer());
			}
			...
			if (filterString != null) {
				...
				c
						.setFilter(LuceneUtils.parseFilter(filterString,
								filterParser));
			} else {

				String queryFilterString = null;
				if (queryFilterString == null) {
					queryFilterString = request
							.getCleanParameter("searchTermsFilter");
				}
				if (queryFilterString == null) {
					queryFilterString = request
							.getCleanParameter("QueryFilter");
				}
				if (queryFilterString == null) {
					queryFilterString = request
							.getCleanParameter("queryFilter");
				...
				if (queryFilterString != null) {
					c.getLogger().debug(
							"Query filter string: " + queryFilterString);
					c.setFilter(new QueryFilter(filterParser
							.parse(queryFilterString)));
				}
		...
		if (c.getSort() == null) {
			...
			if (sortString == null) {
				sortString = request.getCleanParameter("sort");
			}
	...
	/**
	 * Escapes the characters within a given object's toString() output.
	 * 
	 * @param object
	 *            An object to escape
	...
	/**
	 * Cleans a given string. This involves turning empty strings into nulls.
	 * 
	 * @param string
	 *            A <code>String</code> to clean
	...
	/**
	 * Parses a <code>Locale</code> from the provided string. Essentially
	 * reverse engineers the string based on <code>Locale</code>'s toString
	 * format.
	 * 
	 * @param string
	 *            String to parse <code>Locale</code> from
	 ...
	 * 
	 * @param string
	 *            A String from which to extract directories
	...
	public static void indentElement(int level, Element element,
			Document document) {
	...
	public static void printDocument(Document document, PrintWriter out)
			throws Exception {
		printDocument(document, new StreamResult(out));
	}

	public static void printDocument(Document document, Result result)
			throws Exception {
	...
	public static Document buildExceptionDocument(Exception e, String suggestion) {
		...
		typeElement.appendChild(doc
				.createTextNode(e.getClass().getSimpleName()));
		errorElement.appendChild(typeElement);

		/**
		 * Element classElement = doc.createElement( "class" );
		 * classElement.appendChild( doc.createTextNode(
		 * e.getClass().getCanonicalName() ) ); errorElement.appendChild(
		 * classElement );
		 */
		...
		errorElement
				.appendChild(buildStackTraceElement(e.getStackTrace(), doc));
	...
	public static Element buildStackTraceElement(StackTraceElement[] stack,
		...
		for (int i = 0; i < stack.length; i++) {
			...
			traceElementElement.appendChild(xmlDocument.createTextNode(String
					.valueOf(stack[i])));
	...
	public static void removeNulls(Node node) {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (childNode == null
					|| (childNode.getNodeType() == Node.TEXT_NODE && childNode
							.getNodeValue() == null)) {
	...
	public static String toCamelCase(String... parts) {
		...
		for (int i = 1; i < parts.length; i++) {
			buffer.append(parts[i].substring(0, 1).toUpperCase()
					+ parts[i].substring(1));
		}
	...
	public static Properties asProperties(Element xoxo) {
		char state = 't'; // collecting <dt> initially, will switch between
		// 't' and 'd' (<dd>)
	...
	public static String[] objectsMapped(String method, Object[] objects) {
		...
		for (int i = 0; i < objects.length; i++) {
			try {
				strings[i] = (String) objects[i].getClass().getMethod(method)
						.invoke(objects[i]);
	...
	public static String asHTTPDate(Calendar calendar) {
		...
		format.format(calendar.getTime(), buffer, new FieldPosition(
