...
import net.lucenews.controller.ExceptionController;
import net.lucenews.controller.FacetController;
import net.lucenews.controller.IndexController;
import net.lucenews.controller.IndexPropertiesController;
import net.lucenews.controller.OpenSearchController;
import net.lucenews.controller.SearchController;
import net.lucenews.controller.ServiceController;
import net.lucenews.controller.ServicePropertiesController;
...
import net.lucenews.model.exception.DocumentsAlreadyExistException;
...
public class LuceneWebService extends HttpServlet {
	...
	private static final long serialVersionUID = 8715325046726469897L;
	...
	public void init() {
		...
		try {
			...
			if (propertiesFile == null) {
				propertiesFile = getProperty("properties.file");
			}
			if (propertiesFile == null) {
				propertiesFile = getProperty("properties-file");
			}
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		c.getLogger().info(
				"request:  " + req.getMethod() + " " + req.getLocation() + " "
						+ req.getProtocol());

		/*
		 * Logger.getLogger(this.getClass()).debug("getContextPath(): " +
		 * request.getContextPath());
		 * Logger.getLogger(this.getClass()).debug("getPathInfo(): " +
		 * request.getPathInfo());
		 * Logger.getLogger(this.getClass()).debug("getPathTranslated(): " +
		 * request.getPathTranslated());
		 * Logger.getLogger(this.getClass()).debug("getQueryString(): " +
		 * request.getQueryString());
		 * Logger.getLogger(this.getClass()).debug("getRequestURI(): " +
		 * request.getRequestURI());
		 * Logger.getLogger(this.getClass()).debug("getRequestURL(): " +
		 * request.getRequestURL());
		 * Logger.getLogger(this.getClass()).debug("getServletPath(): " +
	 ...
	 * 
	 * @param c
	 *            The <tt>LuceneContext</tt> of the request
	...
	public void doDelete(LuceneContext c) throws TransformerException,
	...
	public void doGet(LuceneContext c) throws ParserConfigurationException,
			IndicesNotFoundException, TransformerException, IOException,
			ParseException, LuceneException, OpenSearchException, Exception {
		c.getLogger().trace("doGet");
		LuceneRequest request = c.getRequest();

		c.getLogger().debug(
				"request has " + request.getIndexNames().length
						+ " index names");
		...
		if (request.getDocumentIDs().length == 1) {
			...
			if (request.getDocumentID().equals("opensearchdescription.xml")
					|| request.getDocumentID().equals("description.xml")) {
			...
			/**
			 * // JavaScript suggestions if (
			 * request.getDocumentID().equals("suggest") ) {
			 * SearchController.doSuggest( c ); return; }
			 */
		...
		if (request.hasIndexNames()) {
			...
			} else if (c.getOpenSearchQuery() != null
	...
	public void doHead(LuceneContext c) throws ParserConfigurationException,
			IndicesNotFoundException, TransformerException, IOException,
			ParseException, LuceneException, OpenSearchException, Exception {
	...
	public void doPost(LuceneContext c) throws IllegalActionException,
			DocumentsAlreadyExistException, ParserConfigurationException,
			IndicesAlreadyExistException, TransformerException,
			AtomParseException, IndicesNotFoundException, SAXException,
			IOException, LuceneException {
	...
	public void doPut(LuceneContext c) throws IllegalActionException,
		...
		if (request.getDocumentIDs().length == 0) {
			if (request.getIndexNames().length == 1
					&& request.getIndexName().equals("service.properties")) {
				ServicePropertiesController.doPut(c);
				return;
			}
			if (request.getIndexNames().length > 0
					&& request.getParameter("optimize") != null) {
	...
	/**
	 * Stores the current properties to the designated properties file.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Stored the current properties to the designated properties file using the
	 * provided comment.
	 * 
	 * @param comment
	 *            A comment to include in the file
	 * @throws IOException
	 */
	...
	/**
	 * Sets the Lucene web service properties to the given set of properties.
	 * 
	 * @param properties
	 *            A set of properties
	 * @throws IOException
	 ...
	 * @throws IOException
	 *             if index manager could not refresh
	 */
	...
	/**
	 * Sets the current properties as the contents of the provided file.
	 * 
	 * @param file
	...
	/**
	 * Loads properties from provided file (assumed to be stored in XML format.
	 * 
	 * @param file
	 *            The file to load from
	 * @throws IOException
	...
	public void propertiesChanged() throws IOException {
		...
		if (getProperty("log4j.configuration") != null) {
			PropertyConfigurator
					.configureAndWatch(getProperty("log4j.configuration"));
		}

		/**
		 * String writeLockTimeout =
		 * getProperty("indexwriter.writelocktimeout"); if ( writeLockTimeout !=
		 * null ) { IndexWriter.setDefaultWriteLockTimeout( Long.valueOf(
		 * writeLockTimeout ) ); }
		 */
	}
	...
	/**
	 * Retrieves a specific property, defaulting to the second parameter if such
	 * a property does not exist.
	 * 
	 * @param name
	 *            The name of desired property
	 * @param defaultValue
	 *            The value returned if the appropriate property does not exist
	 * @throws IOException
	 */
	...
	@Deprecated
	public static String getIndicesURL(LuceneRequest request,
			LuceneIndex... indices) {
	...
	public static HttpURI getIndicesURI(LuceneRequest request,
			String... indices) {
		StringBuffer buffer = new StringBuffer();
		for (int i = 0; i < indices.length; i++) {
			if (i > 0) {
	...
	@Deprecated
	public static String getIndexPropertiesURL(LuceneRequest request,
			String indexName) {
		return getIndexURL(request, indexName) + "/index.properties";
	}

	public static HttpURI getIndexPropertiesURI(LuceneRequest request,
			String indexName) {
	...
	@Deprecated
	public static String getDocumentURL(LuceneRequest request,
			String indexName, String documentID)
			throws InsufficientDataException, IOException {
		return getIndexURL(request, indexName) + "/" + documentID;
	}

	public static HttpURI getDocumentURI(LuceneRequest request,
			String indexName, String documentID)
			throws InsufficientDataException, IOException {
	...
	@Deprecated
	public static String getDocumentURL(LuceneRequest request,
			LuceneIndex index, LuceneDocument document)
			throws InsufficientDataException, IOException {
		return getDocumentURL(request, index.getName(), index
	...
	public static HttpURI getDocumentURI(LuceneRequest request,
			LuceneIndex index, LuceneDocument document)
			throws InsufficientDataException, IOException {
		return getDocumentURI(request, index.getName(), index
				.getIdentifier(document));
	}
	...
	@Deprecated
	public static String getOpenSearchDescriptionURL(LuceneRequest request,
			String... indices) {
		String s = getServiceURL(request);
		return s.substring(0, s.length() - 1)
				+ getOpenSearchDescriptionPathInfo(indices);
	}

	public static HttpURI getOpenSearchDescriptionURI(LuceneRequest request,
			String... indices) {
	...
	/**
	 * Determines the last time any of this service's indices have been updated.
	 * 
	 * @throws IndicesNotFoundException
	 * @throws IOException
	 */
	...
	public Calendar getUpdated() throws IndicesNotFoundException, IOException {
		...
		for (int i = 0; i < indices.length; i++) {
			...
			if (updated == null
					|| updated.compareTo(index.getLastModified()) < 0) {
	...
	/**
	 * Determines the title of this service as specified by the service.title
	 * service property. Defaults to "Lucene Web Service (vX.X.X)".
	 */
	public String getTitle() throws IOException {
		...
		String title = null;
		if (title == null) {
			title = getProperty("service.title");
		}
		if (title == null) {
	...
	/**
	 * Determines the service-wide default field to be used as a last resort
	 * when searching.
	 * 
	 * @throws IOException
	 */
	...
	public String[] getDefaultFields() throws IOException {
		...
		if (defaultFields == null) {
			defaultFields = getProperty("service.field.<default>");
		}
		if (defaultFields == null) {
			defaultFields = getProperty("service.field.default");
		}
		if (defaultFields == null) {
			defaultFields = getProperty("service.defaultfield");
		}
	...
	/**
	 * Determines the service-wide default operator to be used as a last resort
	 * when searching.
	 * 
	 * @throws IOException
	 */
	...
	public QueryParser.Operator getDefaultOperator() throws IOException {
		...
		if (defaultOperator == null) {
			defaultOperator = getProperty("service.operator.<default>");
		}
		if (defaultOperator == null) {
			defaultOperator = getProperty("service.operator.default");
		}
		if (defaultOperator == null) {
			defaultOperator = getProperty("service.defaultoperator");
		}
		if (defaultOperator == null) {
			defaultOperator = getProperty("service.operator");
		}
	...
	public boolean isDebugging() throws IOException {
		...
		if (debug == null)
			debug = getProperty("debugging");
		if (debug == null)
			debug = getProperty("service.debugging");
	...
	public OpenSearchImage getImage() throws NumberFormatException, IOException {
		...
		String url = null;
		if (url == null) {
			url = getCleanProperty("service.image.url");
		}
		if (url == null) {
			url = getCleanProperty("service.image");
		}

		if (url == null) {
			image
					.setUrl("http://www.lucene-ws.net/images/magnifying_glass.png");
			return image;
		}
