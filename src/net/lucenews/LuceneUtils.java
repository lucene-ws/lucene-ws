...
import net.lucenews.model.DateComparator;
...
public class LuceneUtils {
	 ...
	 * @param string
	 *            The string to parse
	 * @return A new instance of the class corresponding to the given string.
	 *         Null if invalid
	 */

	public static Analyzer parseAnalyzer(String string) {
		String[] possibleClassNames = new String[] { string,
		...
		for (int i = 0; i < possibleClassNames.length; i++) {
			...
			try {
				return (Analyzer) Class.forName(possibleClassName)
						.newInstance();
			} catch (Exception e) {
			}
		}
	...
	public static SortComparatorSource parseSortComparatorSource(String string) {
		...
		for (int i = 0; i < possibleClassNames.length; i++) {
			String possibleClassName = possibleClassNames[i];
			try {
				return (SortComparatorSource) Class.forName(possibleClassName)
						.newInstance();
			} catch (Exception e) {
			}
		}
	...
	/**
	 * Parses the operator described by the string provided. This corresponds to
	 * the QueryParser.AND_OPERATOR and QueryParser.OR_OPERATOR values.
	 * 
	 * @param string
	 *            The string to parse
	 * @return The QueryParser.Operator value of the operator, Null if the
	 *         operator is invalid
	 */
	...
	public static Filter parseFilter(String string, QueryParser parser)
		...
		if (string.startsWith("QueryFilter:")) {
			return new QueryFilter(parser.parse(string.substring("QueryFilter:"
					.length())));
		}
	...
	public static Locale parseSortLocale(String string) {
		...
		if (string.matches(".+\\(\\w+\\)!?")) {
			String localeString = string.substring(string.indexOf("(") + 1,
					string.indexOf(")"));
	...
	/**
	 * Parses an individual sort field. Acts as a helper to to the getSort()
	 * method.
	 */
	...
	public static SortField parseSortField(String string) {
		 ...
		 * Document The document sort field is indicated by the presence of '<doc>' -
		 * Score The score sort field is indicated by the presence of '<score>' -
		 * Custom A custom sort field is indicated by the presence of '<custom:
		 * ... >' - User-defined A user defined sort field is indicated by the
		 * presence of '"$name"'
		 * 
		 */
		...
		if (string.matches("^<.+>.*")) {
			...
			if (substring.equals("custom")) {
				...
				return new SortField(fieldName, LuceneUtils
						.parseSortComparatorSource(factory));
			}
		}
	 ...
	 * Sort field order
	 * 
	 * A reversed sort order is indicated by an exclamation mark located at the
	 * end of the sort string. Its absense implies a forward sort order (the
	 * default).
	 */
	 ...
	 * Sort field type
	 * 
	 * The field being sorted on may be interpreted in light of a specific data
	 * type. A type definition is indicated by the presence of '[$type]' where
	 * $type may take on any of the following values: - int - string - float -
	 * auto
	 * 
	 */
	...
	public static int parseSortType(String string) {
		...
		if (string.matches("^.+\\[.+\\].*$")) {
			String substring = string.substring(string.indexOf("[") + 1,
					string.indexOf("]")).toLowerCase().trim();
